...
export const SIGN_IN_RESULTS = "SIGN_IN_RESULTS";
export const signInResults = (data) => {
    return { type: SIGN_IN_RESULTS, data }
}
...
export const signUpForm = (values) => {
     return dispatch => {
        axios.post(`/api/users/sign-up`, values)
            .then(res => console.log(res))
    }
}

export const SIGN_IN_FORM = "SIGN_IN_FORM";
export const signInForm = (values) => {
     return dispatch => {
        axios.post(`/api/users/sign-in`, values)
            .then(res => dispatch(signInResults(res.data)))
    }
}

export const githubClick = () => {
    return dispatch => {
        axios.get('/api/users/auth/github')
            .then(res => dispatch(dbTestResults(JSON.stringify(res.data))))
            .catch(err => dispatch(dbTestError(err)))
    }
}
